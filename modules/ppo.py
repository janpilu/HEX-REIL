...
class PPOMemory:
    ...
    def store_memory(self, action, probs, values, reward, done):
...
class PPOActor:
    def __init__(
        ...
        self.optimizer = optim.AdamW(self.actor.parameters(), lr=alpha)
...
class PPOCritic(nn.Module):
    def __init__(
        self, input_dims, alpha, fc1_dims=256, fc2_dims=256, checkpoint_dir="tmp/ppo"
    ):
        ...
        self.optimizer = optim.AdamW(self.critic.parameters(), lr=alpha)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.to(self.device)
...
class PPO:
    def __init__(
        ...
        self.actor = PPOActor(
            n_actions, input_dims, alpha=alpha, fc1_dims=fc1_dims, fc2_dims=fc2_dims
        )
        self.critic = PPOCritic(
            input_dims, alpha=alpha, fc1_dims=fc1_dims, fc2_dims=fc2_dims
        )
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    def remember(self, state, action, probs, values, reward, done):
        self.memory.store_memory(action, probs, values, reward, done)
    ...
    def choose_action(self, observation):
        state = torch.tensor([observation], dtype=torch.int8).to(self.device)
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            for batch in batches:
                states = torch.tensor(state_arr[batch], dtype=torch.int8).to(
                    self.actor.device
                )
