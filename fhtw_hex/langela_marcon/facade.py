...
def agent(board, action_set):
    agent_dir = "./fhtw_hex/langela_marcon/models/7x7/winner/final_submission"
    agent = None

    agent_file = os.listdir(agent_dir)[0]

    model, layers, size, architecture = agent_file.split("_")

    hidden_layers = int(layers)
    hidden_size = int(size)
    use_conv = architecture == "conv"
    ...
    agent.set_env(env)
    agent.load(f"{agent_dir}/{agent_file}")
